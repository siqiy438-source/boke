...
export const TimelineView: React.FC<TimelineViewProps> = ({ posts, onCardClick }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12 animate-in fade-in slide-in-from-bottom-4 duration-500 motion-reduce:animate-none">
                ...
                <div className="space-y-8">
                  {monthPosts.map((post, index) => (
                    <div key={post.id} className="relative group">
                      {/* 时间线上的小圆点 - 渐变发光效果 */}
                      <div className="absolute -left-8 top-1/2 -translate-y-1/2 z-10 group-hover:scale-125 transition-all duration-300 motion-reduce:transition-none motion-reduce:group-hover:scale-100">
                        <div className="absolute inset-0 w-5 h-5 rounded-full bg-brand-orange blur-md opacity-60 group-hover:opacity-80 motion-reduce:group-hover:opacity-60"></div>
                      ...
                      <div
                        ...
                        className="pl-6 cursor-pointer group-hover:translate-x-2 transition-transform duration-300 motion-reduce:transition-none motion-reduce:group-hover:translate-x-0"
                        role="article"
                      >
                        ...
                        <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-300 border border-stone-200/50 dark:border-slate-700/50 group-hover:border-brand-orange/50 motion-reduce:transition-none">
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                              ...
                              <h3 className="text-lg font-bold text-slate-900 dark:text-stone-100 mb-2 group-hover:text-brand-orange transition-colors font-serif motion-reduce:transition-none">
                                {post.title}
                              </h3>
                ...
                {monthPosts.map((post) => (
                  <div key={post.id} className="relative group">
                    ...
                    >
                      ...
                      <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-xl p-4 shadow-md hover:shadow-lg transition-all duration-300 border border-stone-200/50 dark:border-slate-700/50 motion-reduce:transition-none">
                        <span className="inline-block px-2 py-0.5 rounded-md text-xs font-semibold bg-brand-orange/10 text-brand-orange mb-2">
                          {post.category}
                        </span>
                        <h3 className="text-base font-bold text-slate-900 dark:text-stone-100 mb-1 group-hover:text-brand-orange transition-colors font-serif motion-reduce:transition-none">
                          {post.title}
                        </h3>
                        <p className="text-sm text-slate-600 dark:text-stone-400 line-clamp-2">
                          {post.excerpt}
